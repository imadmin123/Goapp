...
#작성자와 설명을 작성한다.
MAINTAINER choi.ss <hylee@dshub.cloud>
LABEL "purpose"="Service Deployment using Multi-stage builds."

#/usr/src/goapp 경로로 이동한다.
WORKDIR /usr/src/goapp

#현재 디렉토리의 goapp.go 파일을 이미지 내부의 현재 경로에 복사 한다.
COPY goapp.go .

#Go언어 환경 변수를 지정하고 /usr/local/bin 경로에 gostart 실행파일을 생성한다.
RUN CGO_ENABLED=0 GOOS=linux GOARCH=am64 go build -o /usr/local/bin/gostart
